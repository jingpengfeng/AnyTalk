				...
				if (i > 0) {
					...
					if (titles[i].equals("用户名")) {
						LinearLayout layout = (LinearLayout) LayoutInflater
								.from(getActivity()).inflate(R.layout.msg_item,
										null);
						layout.setBackgroundDrawable(getOtherButtonBg(titles, i));
						mPanel.addView(layout);
					} else {
						mPanel.addView(bt);
					}
